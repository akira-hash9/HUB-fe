const API_URL = 'https://hub-fe-k5qy.onrender.com/ask';
...
async function carregarSugestoes() {
  ...
  try {
    ...
    if (faq.length >= 3) perguntas = faq.map(f => f.texto);
  } catch (_) {}
  ...
  label.style.cssText = `
    font-size:10px; color:rgba(200,181,255,0.5);
  ...
  perguntas.slice(0, 5).forEach((pergunta, i) => {
    ...
    btn.style.cssText = `
      background:rgba(200,181,255,0.07); border:1px solid rgba(200,181,255,0.15);
      border-radius:20px; color:#f0eff5; font-size:12px; padding:6px 12px;
      text-align:left; cursor:pointer;
      transition:background 0.2s ease, border-color 0.2s ease, transform 0.2s ease;
      opacity:0; animation:fadeUp 0.3s ease forwards; animation-delay:${i * 0.06}s;
    `;
    btn.onmouseenter = () => {
...
async function sendMessage() {
  ...
  chatMessages.innerHTML += `
    <div id="${loadingId}" style="margin-bottom:12px; font-size:13px; color:var(--accent); opacity:0.8;">
      G AI está pensando...
    </div>`;
  ...
  try {
    ...
    if (el) el.innerText = 'Erro: servidor offline.';
...
if (ageConfirmBtn) {
  ageConfirmBtn.addEventListener('click', () => {
    if (pinterestUrl) window.open(pinterestUrl, '_blank');
    ageWarningModal.classList.add('hidden');
    ageWarningOverlay.classList.add('hidden');
    pinterestUrl = '';
  });
...
setTimeout(() => {
  ...
  bubble.style.cssText = `
    ...
    background:rgba(15,15,20,0.95); border:1px solid rgba(200,181,255,0.3);
    border-radius:16px 16px 4px 16px; padding:10px 14px;
    font-size:13px; color:#f0eff5; z-index:999;
...
if (avatar && avatarImg) {
  ...
  avatar.addEventListener('click', () => {
    ...
    overlay.style.cssText = `
      ...
      transition:background 0.3s ease; cursor:zoom-out;
    `;
...
<div style="display:flex;flex-direction:column;gap:6px;font-size:11px;">
  <div style="display:flex;align-items:center;gap:8px;"><span style="display:inline-block;width:12px;height:12px;border-radius:3px;background:#0a0a0f;border:1px solid rgba(255,255,255,0.1);"></span><code style="color:#85e3c8;">--bg</code><span style="color:#c8c8d8;">#0a0a0f</span></div>
  <div style="display:flex;align-items:center;gap:8px;"><span style="display:inline-block;width:12px;height:12px;border-radius:3px;background:#111118;border:1px solid rgba(255,255,255,0.1);"></span><code style="color:#85e3c8;">--surface</code><span style="color:#c8c8d8;">#111118</span></div>
  <div style="display:flex;align-items:center;gap:8px;"><span style="display:inline-block;width:12px;height:12px;border-radius:3px;background:#f0eff5;"></span><code style="color:#85e3c8;">--text</code><span style="color:#c8c8d8;">#f0eff5</span></div>
  <div style="display:flex;align-items:center;gap:8px;"><span style="display:inline-block;width:12px;height:12px;border-radius:3px;background:#c8b5ff;"></span><code style="color:#85e3c8;">--accent</code><span style="color:#c8c8d8;">#c8b5ff</span></div>
  <div style="display:flex;align-items:center;gap:8px;"><span style="display:inline-block;width:12px;height:12px;border-radius:3px;background:#85e3c8;"></span><code style="color:#85e3c8;">--accent2</code><span style="color:#c8c8d8;">#85e3c8</span></div>
</div>
<hr style="border:none;border-top:1px solid rgba(200,181,255,0.1);margin:12px 0;">
...
<div style="color:#c8c8d8;font-size:12px;line-height:1.7;">
  Chat via Groq com <code style="background:rgba(200,181,255,0.1);padding:1px 5px;border-radius:4px;color:#85e3c8;">llama-3.1-8b-instant</code>. O comportamento é definido pelos <code style="background:rgba(200,181,255,0.1);padding:1px 5px;border-radius:4px;color:#85e3c8;">.txt</code> em <code style="background:rgba(200,181,255,0.1);padding:1px 5px;border-radius:4px;color:#85e3c8;">bio/</code>.
</div>
<div style="margin-top:10px;background:rgba(200,181,255,0.05);border:1px solid rgba(200,181,255,0.1);border-radius:8px;padding:10px;font-size:11px;color:#85e3c8;line-height:1.8;">
  Você digita → script.js → POST /ask → server.js → Groq → resposta
</div>
<hr style="border:none;border-top:1px solid rgba(200,181,255,0.1);margin:12px 0;">
...
node server.js</pre>
<hr style="border:none;border-top:1px solid rgba(200,181,255,0.1);margin:12px 0;">
<div style="color:rgba(200,181,255,0.4);font-size:11px;">MIT — <a href="https://github.com/akira-hash9" target="_blank" style="color:#c8b5ff;text-decoration:none;">@fe_kl9</a></div>
`;
...
readmeToggle.style.cssText = `
  background:rgba(15,15,20,0.95); border:1px solid rgba(200,181,255,0.2);
  border-radius:8px; color:rgba(200,181,255,0.7);
  font-size:11px; font-family:'Courier New',monospace;
  padding:6px 10px; cursor:pointer; backdrop-filter:blur(10px);
...
readmeBox.style.cssText = `
  background:rgba(10,10,15,0.97); border:1px solid rgba(200,181,255,0.15);
  border-radius:12px; padding:16px; margin-top:8px;
  width:min(300px,80vw); max-height:65vh; overflow-y:auto;
  -webkit-overflow-scrolling:touch;
  backdrop-filter:blur(20px); -webkit-backdrop-filter:blur(20px);
  box-shadow:0 20px 60px rgba(0,0,0,0.5);
  font-family:'Courier New',monospace; font-size:12px; line-height:1.7; color:#c8c8d8;
  opacity:0; transform:translateY(-8px) scaleY(0.95); transform-origin:top;
